...
항공사마다 천장이 다르다. 둘 다 결제는 사람이 한다 — 예약 후 성명 변경이 불가하고
마일리지·현금이 실제로 나가는 단계다.

| | 자동으로 가는 곳 | 사람이 해야 하는 것 |
|---|---|---|
| 대한항공 | 운임(좌석등급) 선택 | 승객 정보, 결제 |
| 아시아나 | **결제 화면** — 운임 선택 + 가족 마일리지 공제 + 탑승자 지정까지 | **보안문자**, 연락처, 결제 |

실측 도달 화면:
- 대한항공 `ICN→NRT 2027-08-05 KE703 프레스티지석 · 32,500마일 + 90,000원`
- 아시아나 `ICN→NRT 2026-12-10 OZ102 비즈니스 2명 · 45,000마일 + KRW 201,000`
  (탑승 본인+배우자 / 공제 본인+어머니)

### 10.5 아시아나의 차이 — 보안문자와 1~2분짜리 세션

**아시아나는 끝까지 자동화되지 않는다.** 두 가지 벽이 있고, 둘 다 실측으로 확인했다.

1. **보안문자(CAPTCHA)** — `#cnCaptchaCode`(6자리). `공제 및 탑승자 입력 완료`를 누르면
   페이지가 직접 말한다: *"보안을 위해 아래 캡차 인증을 진행 해 주시기 바랍니다."*
   즉 **"다 눌러놓고 보안문자만 입력"은 순서상 불가능하다.** 보안문자가 먼저다.
   대한항공에는 이 단계가 없다.
2. **세션 타이머** — 결제 화면에 `남은 시간 N분 N초` 카운트다운이 있고, 실측 시
   갓 연 화면이 **1분 12초** 남아 있었다. 만료되면 첫 화면으로 튕기고 입력이 전부 날아간다.
   `세션 연장` 버튼으로 늘릴 수 있다.

이 두 가지가 설계를 바꾼다. **미리 결제 화면을 띄워놓고 기다리는 전략은 쓸 수 없다** —
세션이 먼저 죽는다. 자리가 열린 그 순간에 빠르게 도달하는 것이 유일한 방법이고,
그래서 조회 속도가 곧 성패다(§10.7).

폼 자체도 대한항공보다 단계가 많다 — 좌석등급을 **반드시** 골라야 하고, `항공권 조회`가
안내 레이어를 열 뿐이라 **그 레이어의 확인**이 실제 제출이다.

### 10.7 아시아나 예매 도달 시간 (2026-09-20 실측)

고정 대기를 전부 "실제 조건 대기"로 바꿔 **27.7초 → 12.8초**로 줄였다.
단계별 측정은 `AWARD_TIMING=1` 환경변수로 켠다.

| 단계 | 개선 전 | 개선 후 | 무엇을 기다리게 바꿨나 |
|---|---|---|---|
| 폼 열기 | 2.6초 | 1.7초 | `#txtDepartureAirport1`이 보일 때까지 |
| 편도 | 1.3초 | 0.3초 | 출발지 칸이 입력 가능해질 때까지 |
| 출발·도착지 | 3.9초 | 1.7초 | 숨은 필드 `#departureAirport1`에 코드가 들어올 때까지 |
| 날짜 | 3.4초 | 0.6초 | `#departureDate1`에 값이 들어올 때까지 |
| 인원 | 1.5초 | 0.2초 | `#adultCount`가 목표값이 될 때까지 |
| 좌석등급 | 1.3초 | 0.2초 | `#btn_coupon_layer`가 보일 때까지 |
| 조회 | 1.4초 | 0.5초 | 안내 레이어의 확인 버튼이 뜰 때까지 |
| 결과 표 | 4.7초 | 4.2초 | (대부분 아시아나 서버 시간) |
| 운임+결제화면 | 7.7초 | 3.4초 | 탑승자 표가 그려질 때까지 |
| **합계** | **27.7초** | **12.8초** | |

서버 API 경유 시 16.9초. 3회 반복(2명 예매·2명 조회·1명 예매) 10.8~13.4초로 안정적.
남은 시간의 대부분은 아시아나 서버가 실제로 쓰는 시간이라 더 줄일 여지가 적다.

### 10.8 가족 마일리지 공제 (`data/local/asiana-booking.json`)

아시아나는 한 예약에 두 명을 넣고, 본인 마일리지가 모자라면 가족 합산으로 채운다.
예매 화면의 스위치는 `#mile_<회원번호>`(공제) / `#boarding_<회원번호>`(탑승)이고,
**공제를 먼저 켜야** 나머지 사람의 탑승 칸이 열린다(그 전에는 "탑승 불가").
본인 행은 `disabled`로 고정되어 있다.

누구 마일리지를 쓰고 누가 타는지는 `data/local/asiana-booking.json`에서 읽는다:

```json
{ "deductFrom": ["<마일리지 빌려줄 가족 회원번호>"],
  "boarding":   ["<본인 외 탑승할 가족 회원번호>"] }
```

**가족 회원번호는 개인정보라 저장소에 넣지 않는다.** `data/`는 `.gitignore`에 있다.
파일이 없거나 깨져 있으면 아무것도 선택하지 않고 항공사 기본값으로 둔다 — 실패가 아니다.
회원번호 형식이 아닌 값은 버린다(선택자에 그대로 들어가면 안 되므로).
